...
public struct Sysctl {
    ...
    /// e.g. "8" or "2"
    public static var activeCPUs: Int64 { return (try? sysctlInt(levels: CTL_HW, HW_AVAILCPU)) ?? 0 }
    ...
    public static var osRev: Int64 { return (try? sysctlInt(levels: CTL_KERN, KERN_OSREV)) ?? 0 }
    ...
    public static var cpuFreq: Int64 { return (try? sysctlInt(name: "hw.cpufrequency")) ?? 0 }
    
    /// e.g. 25769803776 (not available on iOS)
    public static var memSize: Int64 { return (try? sysctlInt(levels: CTL_HW, HW_MEMSIZE)) ?? 0 }
    #endif
}
