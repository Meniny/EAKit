...
public typealias void_block_t = @convention(block) () -> Swift.Void

// MARK: - Properties
/// EAKit: Common usefull properties and methods.
public struct EAKit {
	
	#if !os(macOS)
	/// EAKit: App's name (if applicable).
	...
	#if !os(macOS)
	/// EAKit: App's bundle ID (if applicable).
	...
	#if !os(macOS)
	/// EAKit: App current build number (if applicable).
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Application icon badge current number.
	...
	#if !os(macOS)
	/// EAKit: App's current version (if applicable).
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Shared instance of current device.
	...
	#if os(iOS)
	/// EAKit: Current orientation of device.
	...
	/// EAKit: Check if app is running in debug mode.
	...
	#if !os(macOS)
	/// EAKit: Check if app is running in TestFlight mode.
	...
	#if os(iOS)
	/// EAKit: Check if multitasking is supported in current device.
	...
	#if os(iOS)
	/// EAKit: Current status bar network activity indicator state.
	...
	#if os(iOS)
	/// EAKit: Check if device is iPhone.
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Check if device is registered for remote notifications for current app (read-only).
	public static var isRegisteredForRemoteNotifications: Bool {
		return UIApplication.shared.isRegisteredForRemoteNotifications
	}
	#endif
	
	/// EAKit: Check if application is running on simulator (read-only).
	...
	#if os(iOS)
	/// EAKit: Status bar visibility state.
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Key window (read only, if applicable).
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Most top view controller (if applicable).
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Shared instance UIApplication.
	...
	#if os(iOS)
	/// EAKit: Current status bar style (if applicable).
	...
	#if !os(macOS)
	/// EAKit: System current version (read-only).
...
// MARK: - Methods
public extension EAKit {
	
	/// EAKit: Delay function or closure call.
	///
	/// - Parameters:
	...
	/// EAKit: Debounce function or closure call.
	///
	/// - Parameters:
	...
	#if os(iOS) || os(tvOS)
	/// EAKit: Called when user takes a screenshot
	///
	...
	/// EAKit: Class name of object as string.
	///
